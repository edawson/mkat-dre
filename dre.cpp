...
main(int argc, char **argv)
{
    ...
    bool filter_dups_by_flag = false;

	while ((option = getopt(argc, argv, "vhfPms:b:i:d:o:r:")) != -1) {
		switch (option) {
		case 'r':
	...
	cout << "   output file: " << f(outfile_name) << endl
	     ...
	     << "   using PCR dup flag: " << boolalpha << filter_dups_by_flag << endl
    ...
    long num_pcr_marked_dups = 0;  // will only be non-0 if -m option given
	...
	if (verbose)
		cout << "Memorizing clipped discordant reads..." << endl;
     ...
     */
    int num_memorized_clipped_discs = 0;
    map<string, bool> memo_clipped_disc;
	...
	while (reader.GetNextAlignment(al)) {
        ...
        if (filter_dups_by_flag && al.IsDuplicate()) {
            continue;
        }
		string rg;
		al.GetReadGroup(rg);
		...
		if (is_discordant(al, p.first, p.second, nstdevs) && is_clipped(al)) {
            ++num_memorized_clipped_discs;
            memo_clipped_disc[al.Name] = true;
        }
    }
    reset_stats();  /* reset counters associated with is_discordant */
    if (verbose) {
        cout << "recorded " << num_memorized_clipped_discs
             << " clipped discordant read IDs in memory" << endl;
    }

	/* Use the slow method to read tags so we can get the read group */
	if (verbose)
		cout << "Starting final file scan..." << endl;
    reader.Rewind();
	while (reader.GetNextAlignment(al)) {
		++num_in;
        /* Ignore reads flagged as duplicate if option is specified */
        ...
        map<string, bool>::iterator memit = memo_clipped_disc.find(al.Name);
        /* not discordant and not in the memo buffer */
		if (!is_discordant(al, p.first, p.second, nstdevs) &&
            memit == memo_clipped_disc.end())
			continue;
	...
	cout << "discarded " <<
		not_same_chr + not_diff_strands + weird_insert_size + big_insert_size - num_out - num_blacklisted - same_pos
	     << " PCR duplicates" << endl;
    if (filter_dups_by_flag)
        cout << "   -m PCR dups ignored:   " << num_pcr_marked_dups << endl;
	reader.Close();
	writer.Close();
	if (dups != NULL)
		dups->Close();
